...
## Theme Wiring
- `NordThemePlugin::register()` loads `plugins/nord-theme/resources/css/theme.css`; keep that path in sync if the plugin id or directory name changes.
- `plugin.json` currently uses `id: "nord-theme"`, namespace `Boy132\\NordTheme`, and class `NordThemePlugin`; do not rename one without updating all related references.
